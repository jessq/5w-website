...
int main(int argc, char *argv[])
{
  ...
  while (searchscore<=HIGHESTSCORE)
      ...
      for (pos=0;pos<size;pos++)
	{
	  ...
	  score+=exp(-fabs(pow(thisscore-searchscore,2)/variance))/sqrt(variance);
	}
  ...
  votelabel=sprintfalloc("votes: %i",Vector_getSize(stats));
  gdImageString(im,gdFontLarge,x+horizpad+width-(gdFontLarge->w)*strlen(votelabel),2,votelabel,blue);
  free(votelabel);
