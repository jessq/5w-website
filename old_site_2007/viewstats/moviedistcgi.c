...
void readdatafile(Vector stats, int movie);
...
int main(int argc, char *argv[])
{
  ...
  formdata=Table_gets(formt,"MOVIE");

  if (formdata!=NULL)

    {
      movienumber=atoi(formdata);
      readdatafile(stats,movienumber);
    }
...
void readdatafile(Vector stats, int movienumber)
{
  FILE *dfile;
  ...
  while ((line=freadln(dfile))!=NULL)
	...
	continue;
      ...
      if (thismovienumber==movienumber)
	{
